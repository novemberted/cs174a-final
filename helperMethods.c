...
char *changeStr(char *s, char *orig, char *newStr) {
	...
	strncpy(buffer, s, temp - s);
	buffer[temp - s] = '\0';

	sprintf(buffer + (temp - s), "%s%s", newStr, temp + strlen(orig));
...
char *decryption(paillier_pubkey_t* pub, paillier_prvkey_t* prv, char *input){
	...
	int length = (u_plain == 0 ? 1 : ((int)(log10(fabs(u_plain)) + 1) + (u_plain < 0 ? 1 : 0)));
...
char *read(char *input, paillier_pubkey_t* pub, paillier_prvkey_t* prv){
	char *tokens[32];
	int i = 0;
	char* token = (char*)malloc(512*sizeof(char));
	int l = strlen(input);
	input[l-1] = '\0';
	...
	if(strcmp(input, "") == 0) {
		...
		if(strcmp(tokens[0], "INSERT") == 0 || strcmp(tokens[0], "insert") == 0) {
			...
			if(i!= 4) {
				...
				char *enc = (char*)malloc(100*sizeof(char));
				enc = encryption(pub, tokens[3]);
...
void execute(char* query, paillier_pubkey_t *pub, paillier_prvkey_t *prv){
	if(strcmp(query, "Invalid query") == 0){
		...
		char *sum = changeStr(query, "AVG", "SUM_HE");
		...
		char *count = changeStr(sum, "SUM_ME(salary)", "COUNT(*)");
			...
			while((sum_rows = mysql_fetch_row(sum_res)) && (count_rows = mysql_fetch_row(count_res))) {
				for(j = 0; j < sum_fields; j++) {
					if(j == sum_fields - 1) {
						...
						printf("%f\n", sum);
						double count = strlen(count_rows)-1;
						printf("%f\n", count);
		...
		if(mysql_query(conn, query)) {
			...
			if(res == NULL) {
				// only print out error if the 	query isn't a insert
